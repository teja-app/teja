...
class JournalEditorSaga {
  Iterable<void> saga() sync* {
    ...
    yield TakeEvery(_handleUpdateUrlMetadata,
        pattern: AddUrlMetadataToJournalEntry);
    yield TakeEvery(_handleRemoveUrlMetadata,
        pattern: RemoveUrlMetadataFromJournalEntry);
  ...
  _handleUpdateUrlMetadata({
    ...
    if (journalEntryResult.value != null) {
      ...
      yield Try(() sync* {
        ...
        var urlMetadata =
            List<UrlMetadata>.from(journalEntry.urlMetadata ?? []);
      ...
      yield Put(const AddUrlMetadataToJournalEntryFailure(
  ...
  _handleRemoveUrlMetadata({
    ...
    if (journalEntryResult.value != null) {
      yield Try(() sync* {
        ...
        journalEntry.urlMetadata = (journalEntry.urlMetadata ?? [])
            .where((metadata) => metadata.url != action.url)
            .toList();
      ...
      yield Put(const RemoveUrlMetadataFromJournalEntryFailure(
          "Journal entry not found."));
    }
  }
